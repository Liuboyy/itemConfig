    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['developer_group_statistics']);
                }
                ...
                this.GetDeveloperGroupTimeCountInfo(obj).then(({ data}) => {
                    // 导出
            ...
            handleQueryInfoCallBack(data){
                ...
                this.$store.commit('SAVE_QUERY_PARAMS_INFO', {
                    key: 'developer_group_statistics',
                ...
                switch (this.queryData.developmentType) {
                    case '1':
                        this.tableProps = ['name_level_one', ...props, 'avg_develop','avg_inspection','avg_copywriting','avg_photo','avg_art_design1','avg_art_design2','avg_tort_audit','avg_final_instance'];
                        break;
                    case '2':
                        this.tableProps = ['name_level_one', ...props, 'avg_develop','avg_copywriting','avg_photo','avg_art_design2','avg_tort_audit','avg_final_instance'];
                        break;
                    case '4':
                        this.tableProps = ['name_level_one', ...props, 'avg_develop','avg_copywriting','avg_art_design2','avg_tort_audit','avg_final_instance'];
                        break;
                    case '':
                        this.tableProps = ['name_level_one', ...props, 'avg_develop','avg_inspection','avg_copywriting','avg_photo','avg_art_design1','avg_art_design2','avg_tort_audit','avg_final_instance'];
                        break;
        ...
        created(){
            ...
            try{
                getPreDateRange().then((data) => {
                    if (JSON.stringify(this.developerGroupTimeCountInfo) === '{}') {
                        this.queryData.createTime = [data.start, data.end];
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['developer_group_statistics']);
                        this.handleQueryInfoCallBack(this.developerGroupTimeCountInfo);
