<template>
    <div class="ui-main-module">
        ...
        <div class="ui-main-module-table" v-if="saveDateType == 'year'">
            <el-table ref="myTable" :height="tableHeight" :data="tableData" :cellStyle="cellStyle" highlight-current-row
                      stripe border @sort-change="handleSortChange"border>
                <el-table-column
                        v-for="(item, index) in tableFrontKey"
                        :key="index + 'month'"
                        :label="item"
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['product_line']);
                }
            ...
            handleQueryInfoCallBack(data){
                ...
                this.tableProps = ['category_name_level_1','total_value','create_qty', 'create_filed','total_qty','create_sale','quality_sale'];
                ...
                this.tableFixed = this.tableProps.map(item => {
                    if(item === 'category_name_level_1' || item === 'total_value') return true;
                    return false
                });
        ...
        created(){
            ...
            try{
                this.getCurrentMonth().then((data) => {
                    ...
                    if (JSON.stringify(this.developmentProductLineInfo) === '{}') {
                        this.queryData.date = this.saveDefaultDate;
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['product_line']);
                        this.handleQueryInfoCallBack(this.developmentProductLineInfo);
                    }
                });
            }catch (e) {
                console.log(e);
