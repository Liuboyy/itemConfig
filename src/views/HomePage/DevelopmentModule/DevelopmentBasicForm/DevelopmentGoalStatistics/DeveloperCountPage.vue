                ...
                <ul class="ui-main-module-search clearFix">
                    <li>
                        <SearchRequirement label="统计类型">
                            <el-select size="small" v-model="queryData.dimension" @change="handleChangeDimension">
                                <el-option
                    ...
                    <li v-if="queryData.dimension == 'month'">
                        <SearchRequirement label="创建月份">
                            <el-date-picker :clearable="false" size="small" v-model="queryData.date"
                    ...
                    <li>
                        <SearchRequirement label="开发人员">
                            <el-select clearable size="small" v-model="queryData.developer"
                                       collapseTags multiple filterable v-filter>
                                <el-option
                    ...
                    <li>
                        <SearchRequirement label="开发类型">
                            <el-select clearable size="small" v-model="queryData.devType" filterable v-filter>
                                <el-option
                                        v-for="item in optionList.devTypeList"
                                        :key="item.value"
                                        :value="item.value"
                                        :label="item.label"></el-option>
                            </el-select>
                    ...
                    <li>
                        <SearchRequirement label="一级产品线">
                            <el-select clearable size="small" v-model="queryData.productLine"
                                       collapseTags multiple filterable v-filter>
                                <el-option
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['developer_count']);
                }
                ...
                let obj = {
                    ...
                    develop_type: this.queryData.devType,
                ...
                this.GetDeveloperCountInfo(obj).then(({ data}) => {
                    // 导出
            ...
            handleGetSpanArr(data) {
                ...
                data.forEach((item, index) => {
                    if(index == 0) {
                        ...
                        if(item.user_name === data[index - 1].user_name) {
                            this.spanArr[pos] += 1;
                            this.spanArr.push(0);
                        }else {
                            this.spanArr.push(1);
            ...
            handleQueryInfoCallBack(data){
                ...
                //数据渲染
                this.optionList.developerList = data.list.develops;
                this.optionList.devTypeList = data.list.developType;
                this.optionList.productLineList = data.list.products.list;
                ...
                this.tableProps = ['user_name','devp_type_cn','category_name_level_1','total_value','create_qty','create_filed','total_qty','create_sale','quality_sale'];
                ...
                this.tableMinWidth = this.tableProps.map(item => {
                    ...
                    if(item === 'user_name'|| item === 'category_name_level_1') return '100';
                ...
                this.tableFixed = this.tableProps.map(item => {
                    if(item === 'user_name' || item === 'devp_type_cn'|| item === 'category_name_level_1' || item === 'total_value') return true;
                    return false
                });
        ...
        created(){
            ...
            try{
                this.getCurrentMonth().then((data) => {
                    ...
                    if (JSON.stringify(this.developerCountInfo) === '{}') {
                        this.queryData.date = this.saveDefaultDate;
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['developer_count']);
                        this.handleQueryInfoCallBack(this.developerCountInfo);
                    }
                });
            }catch (e) {
                console.log(e);
