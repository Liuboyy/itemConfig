        ...
        <transition name="slide-fade">
            <div class="ui-search-area" v-if="showSearch">
                ...
                <div class="ui-filter-btn">
                    ...
                    <el-button size="small" type="primary" @click="handleQueryInfo({group_id: ''})" icon="icon-chaxun" >查询</el-button>
                </div>
            </div>
        </transition>
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['cost_sku_group']);
                }
                ...
                let obj = {
                    ...
                    group_id: this.queryData.groupTreeId,
                ...
                this.GetCostNumberSkuDevGroupInfo(obj).then(({ data}) => {
                    // 导出
            ...
            handleTableOpen({group_id, id}, treeNode, resolve) {
                this.queryData = JSON.parse(this.queryParamsInfo['cost_sku_group']);
                this.queryData.groupTreeId = group_id;
                let obj = {
                    start_date: this.queryData.createTime[0] || '',
                    end_date: this.queryData.createTime[1] || '',
                    dev_type: this.queryData.developmentType,
                ...
                getCostNumberSkuDevGroupInfo(obj).then(({data}) => {
                    resolve(data.data.value.map((_v, _i) => {
                        this.$nextTick().then(() => {
                            console.log(this.$refs.myTable.handleRefreshTableHeight)
            ...
            handleQueryInfoCallBack(data){
                ...
                this.tableProps = ['group_name','total', 'area5', 'area10', 'area20', 'area50', 'area100', 'area200', 'area350', 'area351'];
                ...
                this.$nextTick().then(() => {
                    this.tableData = data.data.value.map((item, index) => {
                        return {
                            ...item,
                            id: index + 1,
                            hasChildren: item.group_name === '小计' ? false : true
                        }
                    });
                });
            }
        ...
        created(){
            ...
            try{
                ...
                getPreDateRange().then((data) => {
                    if (JSON.stringify(this.costSkuDevelopmentGroupInfo) === '{}') {
                        this.queryData.createTime = [data.start, data.end];
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['cost_sku_group']);
                        this.handleQueryInfoCallBack(this.costSkuDevelopmentGroupInfo);
                    }
                });
            }catch (e) {
                console.log(e);
