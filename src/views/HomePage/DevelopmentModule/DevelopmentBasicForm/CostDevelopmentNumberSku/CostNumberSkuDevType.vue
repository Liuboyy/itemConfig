    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['cost_sku_type']);
                }
                ...
                this.GetCostNumberSkuDevTypeInfo(obj).then(({ data}) => {
                    // 导出
            ...
            handleQueryInfoCallBack(data){
                ...
                this.tableProps = ['devp_type','total','area5', 'area10', 'area20', 'area50', 'area100', 'area200', 'area350', 'area351'];
        ...
        created(){
            ...
            try{
                ...
                getPreDateRange().then((data) => {
                    if (JSON.stringify(this.costSkuDevelopmentTypeInfo) === '{}') {
                        this.queryData.createTime = [data.start, data.end];
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['cost_sku_type']);
                        this.handleQueryInfoCallBack(this.costSkuDevelopmentTypeInfo);
                    }
                });
            }catch (e) {
                console.log(e);
