        ...
        <transition name="slide-fade">
            <div class="ui-search-area" v-if="showSearch">
                <ul class="ui-main-module-search clearFix">
                    ...
                    <li>
                        <SearchRequirement label="SKU销售季度">
                            <el-select
                                    collapseTags
                                    multiple
                                    size="small"
                                    placeholder="选择季度"
                                    v-model="queryData.quarter">
                                <el-option
                                        v-for="(label, value) in optionList.quarterList"
                                        :key="value"
                                        :value="value"
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo (params = {}, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['product_line_quarter']);
                }
                ...
                let obj = {
                    ...
                    sale_time: this.queryData.quarter.length ? this.queryData.quarter.join(',') : this.queryData.salesYear,
            ...
            handleChangeQueryInfo(date) {
                this.getDateOptionListInfo({
                    ...
                    this.optionList.quarterList = data.dates;
        ...
        created () {
            ...
            try {
                getCurrentYear().then((data) => {
                    ...
                    getCurrentDateRange().then((data) => {
                        if (!this.productLineSalesTrendInfo['4'] || JSON.stringify(this.productLineSalesTrendInfo['4']) === '{}') {
                            this.queryData.createTime = [data.start, data.end];
                            this.queryData.inSalesTime = [data.start, data.end];
                            this.queryData.salesYear = this.saveDefaultYear;
                            this.handleChangeQueryInfo(this.queryData.salesYear);
                            ...
                            this.queryData = JSON.parse(this.queryParamsInfo['product_line_quarter']);
                            this.handleChangeQueryInfo(this.queryData.salesYear);
                            this.handleQueryInfoCallBack(this.productLineSalesTrendInfo['4']);
                            if (this.comOptions['quarter_list']) this.optionList = this.comOptions['quarter_list'];
                        }
                    });
                });
            } catch (e) {
