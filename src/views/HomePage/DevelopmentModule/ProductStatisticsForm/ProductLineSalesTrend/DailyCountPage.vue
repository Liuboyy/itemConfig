<template>
    <div class="ui-main-module">
        ...
        <div class="ui-main-module-table">
            ...
            <TableIndex ref="myTable" :tableData="tableData" :tableKey="tableIndexKey" :tableHeight="tableHeight"
                        :showSearch="showSearch"></TableIndex>
            <!--<el-table  border stripe ref="myTable" highlight-current-row-->
                ...
                <!--<el-table-column-->
                    ...
                    <!--<el-table-column label="小计">-->
                        <!--<span slot slot-scope="scope">{{(scope.row[item] && scope.row[item].day) || (scope.row.total.sele == 1 ? '0.0' : '0.00')}}</span>-->
                    <!--</el-table-column>-->
                    <!--<el-table-column label="国内+FBA">-->
                        <!--<span slot slot-scope="scope">{{(scope.row[item] && scope.row[item].site) || (scope.row.total.sele == 1 ? '0.0' : '0.00')}}</span>-->
                    <!--</el-table-column>-->
                    <!--<el-table-column label="海外">-->
                        <!--<span slot slot-scope="scope">{{(scope.row[item] && scope.row[item].oversea) || (scope.row.total.sele == 1 ? '0.0' : '0.00')}}</span>-->
                    <!--</el-table-column>-->
                    <!--<el-table-column label="代销">-->
                        <!--<span slot slot-scope="scope">{{(scope.row[item] && scope.row[item].sale) || (scope.row.total.sele == 1 ? '0.0' : '0.00')}}</span>-->
                    <!--</el-table-column>-->
                <!--</el-table-column>-->
                <!--<template slot="empty">-->
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo (params = {}, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['product_line_day']);
                }
                ...
                let obj = {
                    ...
                    sale_time: this.queryData.salesTime[0] + ',' + this.queryData.salesTime[1],
            ...
            handleResetQueryInfo () {
                ...
                this.queryData.salesTime = [getCountDate(-7), getCountDate(-1)];
...
//            },
            ...
            handleQueryInfoCallBack (data) {
                ...
                this.$nextTick().then(() => {
                    ...
                    this.tableKey.map(item => { return obj[item] = item + '日' });
                    this.tableIndexKey = obj;

                    Object.values(this.tableKey).forEach(item => {
                    ...
                    this.tableData.forEach((item) => { xArr.push(item.linelist_cn_name) });
        ...
        created () {
            ...
            try {
                getCurrentDateRange().then((data) => {
                    if (!this.productLineSalesTrendInfo['1'] || JSON.stringify(this.productLineSalesTrendInfo['1']) === '{}') {
                        this.queryData.salesTime = [getCountDate(-7), getCountDate(-1)];
                        this.queryData.createTime = [data.start, data.end];
                        this.queryData.inSalesTime = [data.start, data.end];
                        this.handleQueryInfo();
                        ...
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['product_line_day']);
                        this.handleQueryInfoCallBack(this.productLineSalesTrendInfo['1']);
                    }
                });
            } catch (e) {
                console.log(e);
