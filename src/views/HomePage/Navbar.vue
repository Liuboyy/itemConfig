    ...
    <div class="ui-nav-bar">
        ...
        <el-menu
            ...
            <template v-for="(item, index) in routerList">
                <el-menu-item :index="index + 1 + ''" v-if="!item.children" class="single_menu_item">
                ...
                <el-submenu :index="index + 1 + ''" v-else>
                    ...
                    <el-menu-item-group v-for="(itemChild, inx) in item.children" :key="inx">
                        <el-menu-item
                                v-if="!itemChild.children || !itemChild.children.length"
                                :index="(index + 1) + '-' + (inx + 1)">
                            <span class="menu-item-dot">
                                <i class="normal-dot" :class="navIndex === (index + 1) + '-' + (inx + 1) ? 'active-dot' : ''"></i>
                            </span><span class="ui-twoChild">{{itemChild.name}}</span>
                        </el-menu-item>
                        <el-submenu
                                v-if="itemChild.children && itemChild.children.length"
                                :index="(index + 1) + '-' + (inx + 1)">
                            <template slot="title">
                                <span class="menu-item-dot">
                                    <i class="normal-dot" :class="autoActiveParentNav((index + 1) + '-' + (inx + 1)) ? 'active-dot' : ''"></i>
                                </span>
                            ...
                            <el-menu-item
                                ...
                                <span class="menu-item-dot">
                                    <i class="normal-dot" :class="autoActiveParentNav((index + 1) + '-' + (inx + 1) + '-' + (i + 1)) ? 'active-dot' : ''"></i></span><span :class="autoActiveParentNav((index + 1) + '-' + (inx + 1) + '-' + (i + 1)) ? 'active' : ''"></span>
