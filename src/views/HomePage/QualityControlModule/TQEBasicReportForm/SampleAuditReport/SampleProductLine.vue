    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['sample_line']);
                }
            ...
            handleQueryInfoCallBack(data){
                ...
                if(this.queryData.tabType == '1'|| this.queryData.tabType == '4') { //日明细
                    ...
                    if(this.queryData.tabType == '1') {
                        ...
                        props.splice(0,2);
                        this.tableProps = ['p_level_name','type_name', ...props];
                        this.tableFixed = [true, true];
                    }
                    if(this.queryData.tabType == '4') {
                        this.tableProps = ['p_level_name','type_name','subtotal', 'january','february','march','april','may','june','july','august','september','october','november','december']
                    }
                }
                if(this.queryData.tabType == '2' || this.queryData.tabType == '3') {//日汇总
                    this.tableData = data.data.value;
                    this.tableProps = ['p_level_name','review_num','reject_num','pass_rate']
                }
        ...
        created(){
            ...
            try{
                getCurrentYear().then((data) => {
                    ...
                    getPreDateRange().then((data) => {
                        if (!this.sampleDevelopmentGroupInfo['line'] || JSON.stringify(this.sampleDevelopmentGroupInfo['line']) === '{}') {
                            this.queryData.year = this.saveEndYear;
                            this.queryData.startYear = this.saveStartYear;
                            this.queryData.endYear = this.saveEndYear;
                            this.queryData.months = this.saveDefaultMonth;
                            ...
                            // 获取保存的查询参数
                            this.queryData = JSON.parse(this.queryParamsInfo['sample_line']);
                            this.handleQueryInfoCallBack(this.sampleDevelopmentGroupInfo['line']);
                        }
                    });
                })
