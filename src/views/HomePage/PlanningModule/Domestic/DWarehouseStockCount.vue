<template>
    <div  class="ui-main-module">
        ...
        <DCountAmountTable :tableData="tableData" :showSearch="showSearch" :otherHeight="['el-tabs__header']"
                           v-if="queryData.query_type === '1'" :isShowAge="isCheckAge" :isShowSale="isCheckSale"></DCountAmountTable>
        <DCountCalculateTable :tableData="tableData" :showSearch="showSearch" :otherHeight="['el-tabs__header']"
                              v-if="queryData.query_type === '2'" :isShowAge="isCheckAge" :isShowSale="isCheckSale"></DCountCalculateTable>
        <DCountSpeciesTable :tableData="tableData" :showSearch="showSearch" :otherHeight="['el-tabs__header']"
                            v-if="queryData.query_type === '3'" :isShowAge="isCheckAge" :isShowSale="isCheckSale"></DCountSpeciesTable>
...
<script>
    ...
    import {
        getWarehouseAmountCountInfo,
        getAmountTitleList,
        getConfirmAmountTitleList,
        getWarehouseCalculateCountInfo,
        getCalculateTitleList,
        getConfirmCalculateTitleList,
        getWarehouseSpeciesCountInfo,
        getSpeciesTitleList,
        getConfirmSpeciesTitleList,
        geOptionList
    } from '@/api/planning_module/d_warehouse_stock_count';

    import {
        getPreDateRange,
    } from '@/api/common/date_format'

    import DCountAmountTable from '../components/DCountAmountTable.vue'
    import DCountCalculateTable from '../components/DCountCalculateTable.vue'
    import DCountSpeciesTable from '../components/DCountSpeciesTable.vue'

    // 组件名称，用于keep-alive和权限（要跟路由保持一致）
    const component_name = 'd_warehouse_stock_count';

    export default {
        name: component_name,
        components: { DCountAmountTable, DCountCalculateTable, DCountSpeciesTable},
        data() {
            return {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['d_warehouse_stock_count']);
                }
                ...
                let fnName = this.queryData.query_type === '1' ? this.getWarehouseAmountCountInfo : this.queryData.query_type === '2' ?
                    this.getWarehouseCalculateCountInfo : this.getWarehouseSpeciesCountInfo;
