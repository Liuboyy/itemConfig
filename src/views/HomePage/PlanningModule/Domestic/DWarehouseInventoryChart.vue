...
<script>
    ...
    const toHtml = (contents) => {
        ...
        return contents.reduce((rhtml, ctn) => {
            if (rhtml) {
                let ctnValue = ctn.seriesName === '缺货率'? (ctn.value || 0) + '%' : (ctn.value || 0);
    ...
    let option = {
        color: ['#78b3fc', '#ff7778', '#77d6c7', '#ff8e3f','#fad44d',],
        title: {
            text: '国内仓库存推移图（单位：万）',
            textStyle: {
                fontSize: 16,
...
//        },
    ...
    import {
        getDWarehouseInventoryChart,
    } from '@/api/planning_module/d_warehouse_inventory_chart';

    const component_name = 'd_warehouse_inventory_chart';
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo (item, isFn, success) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['d_warehouse_inventory_chart']);
                }
                ...
                this.getDWarehouseInventoryChart(obj).then(({data}) => {
            ...
            handleQueryInfoCallBack (data, item, success) {
                ...
                this.$store.commit('SAVE_QUERY_PARAMS_INFO', {
                    key: 'd_warehouse_inventory_chart',
                ...
                if (data.data.chart.length) {
                    ...
                    data.data.chart.forEach(_v => {
                        ...
                        item[3].push(_v.total_fba_money);
                        item[4].push(_v.outstock_rate);
                        item[5].push(_v.m_date);
                    });
                    option.xAxis[0].data = item[5];
                    option.yAxis[1].max = data.data.data_max.outstock_rate;

                    option.legend.data = ['国内仓在库库存', '国内仓采购在途', '国内仓在库+在途库存金额', '汇总国内仓+FBA虚拟仓(在库+在途)', '缺货率'];
