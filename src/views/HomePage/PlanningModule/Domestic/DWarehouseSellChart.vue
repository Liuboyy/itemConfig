...
//        },
    ...
    import {
        getDWarehouseSellChart,
    } from '@/api/planning_module/d_warehouse_sell_chart';

    const component_name = 'd_warehouse_sell_chart';
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo (item, isFn, success) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['d_warehouse_sell_chart']);
                }
                ...
                this.getDWarehouseSellChart(obj).then(({data}) => {
            ...
            handleQueryInfoCallBack (data, item, success) {
                ...
                if (data.data.chart.length) {
                    ...
                    data.data.chart.forEach(_v => {
                        item[0].push(_v.estimated_sale_days);
                        item[1].push(_v.m_date);
                    });
                    option.xAxis[0].data = item[1];
                    option.yAxis[1].max = data.data.data_max.estimated_sale_days;

                    option.legend.data = ['总预计可售卖天数'];
