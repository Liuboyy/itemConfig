<template>
  <div class="ui-main-module">
    ...
    >
       <!-- <template :slot="item" slot-scope="scope" v-for="(item, i) in tableProps" v-if="i > 9 && i <= 13">
        <span class="ui-link" @click="handleClickList(scope.row)">{{scope.row[item]}}</span>
      </template> -->
        

      <template slot="pr_qty" slot-scope="scope">
        <span @click="handleClickDialog(scope.row, 'pr_qty')" class="ui-link">{{scope.row.pr_qty}}</span>
      </template>
      <template slot="wait_pay_onway_qty" slot-scope="scope">
        <span @click="handleClickDialog(scope.row, 'wait_pay_onway_qty')" class="ui-link">{{scope.row.wait_pay_onway_qty}}</span>
      </template>
      <template slot="pur_onway_qty" slot-scope="scope">
        <span @click="handleClickDialog(scope.row, 'pur_onway_qty')" class="ui-link">{{scope.row.pur_onway_qty}}</span>
      </template>
      <template slot="allot_on_way_qty" slot-scope="scope">
        <span @click="handleClickDialog(scope.row, 'allot_on_way_qty')" class="ui-link">{{scope.row.allot_on_way_qty}}</span>
      </template>

     
      <template slot="operation" slot-scope="scope" > 
        <span class="ui-link" @click="handleClick(scope.row)">查看seller_sku</span>
      </template>
    </ComTableInfo>
...
import {
  ...
  getRealtimeList,
  getRealSellerskuList,
  getPickingList,
  getOnwayList,
  getPurchaseOnway,
  getAllotOnway
} from "@/api/sales_module/Real_time_information";
...
const component_name = "real_time_information";
...
export default {
  ...
  data() {
    return {
      ...
      dialogVisible: false,
      dialogTitle: '查看seller_sku',
      dialog: {
          data: [],
          key: ['seller_sku','加权日均销量','备货提前期','安全库存','订购点','缺货量','订购数量','审核状态'],
          props: []
      },
  ...
  methods: {
    ...
    handleQueryInfo(params, isFn) {
      ...
      if (isFn) {
        ...
        this.queryData = JSON.parse(
          this.queryParamsInfo["real_time_information"]
        );
      }
      ...
      this.getRealtimeinformation(obj).then(({ data }) => {
    ...
    //弹框明细
    handleClickDialog(row, type, page, params) {
      ...
      switch(type) {
        case "pr_qty":
         this.dialogTitle = 'PR数量(采购处理中)';
         
          getPickingList(obj).then(({data}) => {
            if(data.status === 2) return;
            if(data.status) {
              this.dialogVisible = true;  
              this.dialog.key = [...Object.values(data.title)];
              this.dialog.props = [...Object.keys(data.title)];
          ...
          case "wait_pay_onway_qty":
          this.dialogTitle = '在途(待财务付款)';
        
          getOnwayList(obj).then(({data}) => {
            if(data.status === 2) return;
            if(data.status) {
              this.dialogVisible = true;  
              this.dialog.key = [...Object.values(data.title)];
          ...
          case "pur_onway_qty":
          this.dialogTitle = '采购在途';
        
          getPurchaseOnway(obj).then(({data}) => {
            if(data.status === 2) return;
            if(data.status) {
              this.dialogVisible = true;  
              this.dialog.key = [...Object.values(data.title)];
          ...
          case "allot_on_way_qty":
          this.dialogTitle = '调拨在途';
        
          getAllotOnway(obj).then(({data}) => {
            if(data.status === 2) return;
            if(data.status) {
              this.dialogVisible = true;  
              this.dialog.key = [...Object.values(data.title)];
    ...
    //获取查看弹框
     handleClick(row, type, page, params) {
         this.saveData.row = row;
         this.dialogTitle = '查看seller_sku';
         this.saveType = type;
         let obj = {
           sku_code: row.sku,
             ...page,
      ...
      getRealSellerskuList(obj).then(({ data }) => {
        if (data.status === 2) return;
        if (data.status) {
            this.dialogVisible = true;
    ...
    handleDetailsPageChange({offset,limit}) {
        ...
        this.handleClick(this.saveData.row, {offset, limit}, {})
    },

    //查看导出
    handleExportDetailsInfo() {
      if (this.saveType) {
        this.handleClickDialog(this.saveData.row, this.saveType, this.saveData.page, {is_excel: 1});
        
      }else {
        this.handleClick(this.saveData.row,this.saveData.page, {is_excel: 1});
      }
    },
