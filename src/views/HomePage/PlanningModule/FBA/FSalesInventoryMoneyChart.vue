        ...
        <TemplateDefault :staticNum="120" color="#FFFFFF">
            ...
            <ComTableInfo
                ...
                <template v-for="(item, index) in tableProps" :slot="item" slot-scope="scope" v-if="index > 0">
                    <span>{{scope.row[item].sales_30d_money}}</span>
                </template>
...
<script>
    ...
    let option = {
        color: ['#fd96c1','#fad44d', '#77d6c7', '#bbaff1','#78b3fc', '#ff7778', '#ff8e3f', '#5fc46f','#FF9F7E','#ff7778'],
        title: {
            text: 'FBA按销量段模拟断货天数的销库金额推移图（单位：万）',
            textStyle: {
                fontSize: 16,
                color: '#333',
            }
...
//        },
    ...
    import {
        getSalesInventoryMoneyChart,
    } from '@/api/planning_module/f_sales_inventory_money_chart';

    const component_name = 'f_sales_inventory_money_chart';
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo (item, isFn, success) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['f_sales_inventory_money_chart']);
                }
                ...
                this.getSalesInventoryMoneyChart(obj).then(({data}) => {
            ...
            handleQueryInfoCallBack (data, item, success) {
                ...
                this.$store.commit('SAVE_QUERY_PARAMS_INFO', {
                    key: 'f_sales_inventory_money_chart',
