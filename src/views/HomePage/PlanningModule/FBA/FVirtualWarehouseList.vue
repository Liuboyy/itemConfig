                ...
                <ul class="ui-main-module-search clearFix">
                    <li>
                        <SearchRequirement label="SKU">
                            <el-input
                                size="small"
                                placeholder="多个查询用英文逗号','隔开"
                                v-model="queryData.sku"
                            ></el-input>
                        </SearchRequirement>
                    </li>
                    <li>
                            ...
                            <el-row>
                                <el-col :span="11">
                                    <el-input
                                        size="small"
                                        placeholder="请输入"
                                        v-model="queryData.procurement_cost_min"
                                    ></el-input>
                                </el-col>
                                ...
                                <el-col :span="11">
                                    <el-input
                                        size="small"
                                        placeholder="请输入"
                                        v-model="queryData.procurement_cost_high"
                                    ></el-input>
                                </el-col>
                            </el-row>
                        </SearchRequirement>
                    ...
                    <li>
                        <SearchRequirement label="产品状态">
                            <el-select
                                size="small"
                                v-model="queryData.product_status"
                                filterable
                                v-filter
                                collapseTags
                                multiple
                            >
                                <el-option
                                    v-for="item in optionList.productStateList"
                                    :key="item.d_code"
                                    :value="item.d_code"
                                    :label="item.d_name"
                                ></el-option>
        ...
        >
            <div class="ui-check_box">
                ...
                <el-checkbox-group v-model="checkTitle" @change="handleCheckedTitleChange">
                    <el-checkbox
                        v-for="(item, value) in allTitleList"
                        :key="value"
                        :label="value"
                        :disabled="value === 'current_inventory'|| value === 'influence'
                                 || value === 'sales_situation'|| value === 'turnover_rate'|| value === 'basic_information'"
                    >{{item}}</el-checkbox>
                </el-checkbox-group>
            </div>
...
import {
    getFVirtualAmountListInfo,
    getFVirtualCalculateListInfo,
    geOptionList,
    getAmountTitleList,
    getConfirmAmountTitleInfo,
    getCalculateTitleList,
    getConfirmCalculateTitleInfo
} from "@/api/planning_module/f_virtual_warehouse_list";

import { getPreDateRange } from "@/api/common/date_format";

import FVListingAmountTable from "../components/FVListingAmountTable.vue";
import FVListingCalculateTable from "../components/FVListingCalculateTable.vue";

// 组件名称，用于keep-alive和权限（要跟路由保持一致）
const component_name = "f_virtual_warehouse_list";

export default {
    name: component_name,
    components: { FVListingAmountTable, FVListingCalculateTable },
    data() {
        return {
            // 初始化实例对象
            thisVm: {},
    ...
    methods: {
        ...
        handleQueryInfo(params, isFn) {
            ...
            if (isFn) {
                ...
                this.queryData = JSON.parse(
                    this.queryParamsInfo["f_virtual_warehouse_list"]
                );
            }
            ...
            let obj = {
                sold_date_start:
                    this.queryData.salesTime && this.queryData.salesTime[0],
                sold_date_end:
                    this.queryData.salesTime && this.queryData.salesTime[1],
            ...
            let fnName =
                this.queryData.query_type === "1"
                    ? this.getFVirtualAmountListInfo
                    : this.getFVirtualCalculateListInfo;
            ...
            fnName().then(({ data }) => {
                if (data.status) {
                    ...
                    this.isIndeterminate =
                        this.checkTitle.length > 0 &&
                        this.checkTitle.length <
                            Object.keys(this.allTitleList).length;
                    this.checkAll = !(
                        this.checkTitle.length > 0 &&
                        this.checkTitle.length <
                            Object.keys(this.allTitleList).length
        ...
        handleCheckedTitleChange(val) {
            this.checkAll =
                val.length === Object.keys(this.allTitleList).length;
            this.isIndeterminate =
                val.length > 0 &&
                val.length < Object.keys(this.allTitleList).length;
        },
