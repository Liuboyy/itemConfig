        ...
        <TemplateDefault :staticNum="120" color="#FFFFFF">
            ...
            <ComTableInfo
                ...
                <template v-for="(item, index) in tableProps" :slot="item" slot-scope="scope" v-if="index > 0">
                    <span>{{scope.row[item].qty_rate + '%'}}</span>
                </template>
...
//        },
    ...
    import {
        getSalesInventoryRatioChart,
    } from '@/api/planning_module/f_sales_inventory_ratio_chart';

    const component_name = 'f_sales_inventory_ratio_chart';
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo (item, isFn, success) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['f_sales_inventory_ratio_chart']);
                }
                ...
                this.getSalesInventoryRatioChart(obj).then(({data}) => {
            ...
            handleQueryInfoCallBack (data, item, success) {
                ...
                this.$store.commit('SAVE_QUERY_PARAMS_INFO', {
                    key: 'f_sales_inventory_ratio_chart',
