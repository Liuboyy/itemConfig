...
<script>
    ...
    let option = {
        color: ['#78b3fc', '#ff8e3f'],
        title: {
            text: 'FBA模拟按断货天数的销库金额及占比推移图（单位：万）',
            textStyle: {
                fontSize: 16,
                color: '#333',
            }
...
//        },
    ...
    import {
        getListingBreakdownMoneyChart,
    } from '@/api/planning_module/f_listing_breakdown_money_chart';

    const component_name = 'f_listing_breakdown_money_chart';
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo (item, isFn, success) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['f_listing_breakdown_money_chart']);
                }
                ...
                this.getListingBreakdownMoneyChart(obj).then(({data}) => {
            ...
            handleQueryInfoCallBack (data, item, success) {
                ...
                this.$store.commit('SAVE_QUERY_PARAMS_INFO', {
                    key: 'f_listing_breakdown_money_chart',
                ...
                if (data.data.chart.length) {
                    ...
                    data.data.chart.forEach(_v => {
                        item[0].push(_v.sales_30d_money);
                        item[1].push(_v.money_rate);
                        item[2].push(_v.m_date);
                    });
                    option.xAxis[0].data = item[2];
                    option.yAxis[1].max = data.data.data_max.money_rate;

                    option.legend.data = ['断货listing销库金额', '占比'];
