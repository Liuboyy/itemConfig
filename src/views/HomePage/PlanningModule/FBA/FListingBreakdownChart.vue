...
//        },
    ...
    import {
        getFListingBreakdownChart,
    } from '@/api/planning_module/f_listing_breakdown_chart';

    const component_name = 'f_listing_breakdown_chart';
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo (item, isFn, success) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['f_listing_breakdown_chart']);
                }
                ...
                this.getFListingBreakdownChart(obj).then(({data}) => {
            ...
            handleQueryInfoCallBack (data, item, success) {
                ...
                if (data.data.chart.length) {
                    ...
                    data.data.chart.forEach(_v => {
                        item[0].push(_v.list_30d_count);
                        item[1].push(_v.count_rate);
                        item[2].push(_v.m_date);
                    });
                    option.xAxis[0].data = item[2];
                    option.yAxis[1].max = data.data.data_max.count_rate;

                    option.legend.data = ['断货listing计数', '占比'];
