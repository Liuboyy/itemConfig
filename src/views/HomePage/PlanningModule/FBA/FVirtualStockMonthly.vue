...
import { getMonthRange } from "../../../../api/common/date_format";

import { getstatiMonthExcelIfon } from "@/api/sales_module/f_virtual_stock_monthly";

import * as importApi from "@/api/import_api/index";

import FVCountperMonthly from "../components/FVCountperDiem";
// 组件名称，用于keep-alive和权限（要跟路由保持一致）
const component_name = "f_virtual_stock_monthly";

export default {
    name: component_name,
    components: { FVCountperMonthly },
    data() {
        return {
            pickerOptions: "",
            // 初始化实例对象
        ...
        handleQueryInfo(params, isFn) {
            ...
            if (isFn) {
                ...
                this.queryData = JSON.parse(
                    this.queryParamsInfo["f_virtual_stock_monthly"]
                );
            }
            ...
            this.getstatiMonthExcelIfon(obj).then(({ data }) => {
