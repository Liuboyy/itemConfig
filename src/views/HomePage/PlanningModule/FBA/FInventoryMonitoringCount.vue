    ...
    <div  class="ui-main-module">
        ...
        <div class="ui-tabs_box">
            <el-tabs v-model="queryData.query_type" type="card" @tab-click="handleTabClick">
                ...
                <el-tab-pane label="种类统计" name="3"></el-tab-pane>
            </el-tabs>
        </div>
        <ComTableInfo
                ref="tableArea"
            ...
            <template slot="sale_day90_sum" slot-scope="scope">
                <div class="text-color">{{scope.row.sale_day90_sum}}</div>
                <div>{{scope.row.sale_day90_rate + '%'}}</div>
            </template>
            <template slot="sale_day180_sum" slot-scope="scope">
                <div class="text-color">{{scope.row.sale_day180_sum}}</div>
                <div>{{scope.row.sale_day180_rate + '%'}}</div>
            </template>
            <template slot="sale_day270_sum" slot-scope="scope">
                <div class="text-color">{{scope.row.sale_day270_sum}}</div>
                <div>{{scope.row.sale_day270_rate + '%'}}</div>
            </template>
            <template slot="sale_day365_sum" slot-scope="scope">
                <div class="text-color">{{scope.row.sale_day365_sum}}</div>
                <div>{{scope.row.sale_day365_rate + '%'}}</div>
            </template>
            <template slot="sale_daydy365_sum" slot-scope="scope">
                <div class="text-color">{{scope.row.sale_daydy365_sum}}</div>
                <div>{{scope.row.sale_daydy365_rate + '%'}}</div>
            </template>

            <template slot="stockage90_zl" slot-scope="scope">
                <div class="text-color">{{scope.row.stockage90_zl}}</div>
                <div>{{scope.row.stockage90_rate + '%'}}</div>
            </template>
            <template slot="stockage180_zl" slot-scope="scope">
                <div class="text-color">{{scope.row.stockage180_zl}}</div>
                <div>{{scope.row.stockage180_rate + '%'}}</div>
            </template>
            <template slot="stockage270_zl" slot-scope="scope">
                <div class="text-color">{{scope.row.stockage270_zl}}</div>
                <div>{{scope.row.stockage270_rate + '%'}}</div>
            </template>
            <template slot="stockage365_zl" slot-scope="scope">
                <div class="text-color">{{scope.row.stockage365_zl}}</div>
                <div>{{scope.row.stockage365_rate + '%'}}</div>
            </template>
            <template slot="stockagedy365_zl" slot-scope="scope">
                <div class="text-color">{{scope.row.stockagedy365_zl}}</div>
                <div>{{scope.row.stockagedy365_rate + '%'}}</div>
            </template>
...
<script>
    ...
    import {
        ...
        getFbaInventoryControlTypeInfo,
        getSiteList,
        getProductStatusList,
        getProductLineList,
        getDevTyoeList
    } from '@/api/planning_module/f_inventory_control_count';
    ...
    const component_name = 'f_inventory_control_count';
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['f_inventory_control_count']);
                }
                ...
                let obj = {
                    ...params,
                    on_sale_start: this.queryData.salesTime && this.queryData.salesTime[0],
                    on_sale_end: this.queryData.salesTime && this.queryData.salesTime[1],
                    develop_time_start: this.queryData.devTime && this.queryData.devTime[0],
                ...
                switch (this.queryData.query_type) {
                    case '1':
                        fnName = this.getFbaInventoryControlAmountInfo;
                        break;
                    case '2':
                        fnName = this.getFbaInventoryControlMeterInfo;
                        break;
                    case '3':
                        fnName = this.getFbaInventoryControlTypeInfo;
                        break;
                }
            ...
            handleQueryInfoCallBack(data) {
                ...
                this.tableMinWidth = this.tableProps.map(item => {
                    if(item === 'is_outstock'|| item === 'index') return '70';
                    if(item === 'stockage90_price'|| item === 'stockage90_stock'|| item === 'stockagedy365_price'|| item === 'stockagedy365_stock'|| item === 'sale_day90_sum'|| item === 'sale_daydy365_sum'|| item === 'stock_zl'||item === 'stockage90_zl'|| item === 'stockagedy365_zl') return '90';
                    if(item === '30_sale_money'|| item === '30_sale_sum'|| item === 'stock_sale_day'|| item === 'available_zl'|| item === 'on_way_zl'|| item === 'available_hdl') return '110';
                    ...
                    if(item === 'stockage180_price'|| item === 'stockage180_stock'|| item === 'sale_day180_sum'|| item === 'stockage180_zl') return '130';
                    if(item === 'stockage270_price'|| item === 'stockage365_price'|| item === 'stockage270_stock'|| item === 'stockage365_stock'|| item === 'sale_day270_sum'|| item === 'sale_day365_sum'|| item === 'stockage270_zl'|| item === 'stockage365_zl') return '140';
                    if(item === 'seller_sku'|| item === 'sale_section') return '200';
                    return '100';
                });
