...
<script>
    ...
    const component_name = 'cd_freight_curve';
    export default {
        name: component_name,
        ...
        methods: {
            ...
            handleQueryInfo (params, isFn, item, success) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['cd_freight_curve']);
                }
                ...
                if (item.key === '0' && type === 'on') {
                    ...
                    for (let i = 0; i < this.chartData.length; i++) {
                        ...
                        if (_v.original_name === item.name) {
                            const delLength = this.handleGetOffCount(item.value);
