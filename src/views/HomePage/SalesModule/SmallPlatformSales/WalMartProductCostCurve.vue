...
<script>
    ...
    import {
        getTreeDataList,
        getWalmartProductCostCurve
    } from '@/api/sales_module/wm_product_cost_curve';
    ...
    const component_name = 'wm_product_cost_curve';
    export default {
        name: component_name,
        ...
        methods: {
            ...
            handleQueryInfo (params, isFn, item, success) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['wm_product_cost_curve']);
                }
                ...
                this.getWalmartProductCostCurve(obj).then(({ data }) => {
                    if (!data.status) {
