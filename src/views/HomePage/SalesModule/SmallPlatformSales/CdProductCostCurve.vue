        ...
        <TemplateDefault :staticNum="120" color="#FFFFFF">
            <div class="ui-content_area clearFix">
                ...
                <div class="ui-content_area--main">
                    <ComListingEchart :ref="item.target" name="productCost" v-for="(item, index) in chartData" width="100%" :key="index" :data="item"></ComListingEchart>
                </div>
            </div>
        </TemplateDefault>
    </div>
...
<script>
    ...
    import {
        getTreeDataList,
        getCdProductCostCurveInfo
    } from '@/api/sales_module/cd_product_cost_curve';
    ...
    const component_name = 'cd_product_cost_curve';
    export default {
        name: component_name,
        ...
        methods: {
            ...
            handleQueryInfo (params, isFn, item, success) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['cd_product_cost_curve']);
                }
                ...
                this.getCdProductCostCurveInfo(obj).then(({ data }) => {
                    if (!data.status) {
