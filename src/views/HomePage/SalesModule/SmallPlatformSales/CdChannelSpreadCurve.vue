                ...
                <ul class="ui-main-module-search clearFix">
                    ...
                    <li>
                        <SearchRequirement label="仓库">
                            <el-select size="small" v-model="queryData.warehouse">
        ...
        <TemplateDefault :staticNum="120" color="#FFFFFF">
            <div class="ui-content_area clearFix">
                ...
                <div class="ui-content_area--main">
                    <CdChannelCurve :ref="item.target" v-for="(item, index) in chartData" width="100%" :key="index" :data="item"></CdChannelCurve>
                </div>
            </div>
        </TemplateDefault>
    </div>
...
<script>
    ...
    import {
        getTreeDataList,
        getCdChannelSpreadCurveInfo
    } from '@/api/sales_module/cd_channel_spread_curve';

    import {
        getMonthRange
    } from '@/api/common/date_format'

    import CdChannelCurve from '../Components/CdChannelCurve.vue'

    // 组件名称，用于keep-alive和权限（要跟路由保持一致）
    const component_name = 'cd_channel_spread_curve';

    export default {
        name: component_name,
        components: { CdChannelCurve },
        data () {
            return {
        ...
        methods: {
            ...
            handleQueryInfo (params, isFn, item, success) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['cd_channel_spread_curve']);
                }
            ...
            handleQueryInfoCallBack (data, item, success, params) {
                ...
                this.optionList.warehouseList = data.warehouse_list;
