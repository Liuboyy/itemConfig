<template>
    <div class="ui-main-module-table" v-if="initTable">
        <el-table ref="myTable" highlight-current-row stripe border :height="tableHeight" :data="tableData" :load="handleTableOpen"
                  row-key="id" lazy style="width: 100%">
            <el-table-column prop="group_name" label="" minWidth="150"></el-table-column>
            <el-table-column title="合计">
            ...
            <el-table-column v-for="(item, index) in tableKey" :key="index" :label="item">
                <el-table-column label="退单金额" minWidth="80">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            handleTableOpen(row, treeNode, resolve) {
                this.$emit('handleTableOpen', row, treeNode, resolve);
            }
        },
        mounted() {
            this.$nextTick().then(() => {
