                    ...
                    <li>
                        <SearchRequirement label="平台">
                            <el-select size="small" v-model="queryData.platform" multiple collapseTags filterable v-filter>
                                <el-option
                                        v-for="(item, value) in optionList.platformList"
...
<script>
    ...
    import {
        getSiteSalesCurveInfo,
        getSiteList,
        getPlatformList
    } from '@/api/sales_module/site_sales_curve';
    import {
        getGroupOptionList
    } from '@/api/common';
    ...
    const component_name = 'site_sales_curve';
    export default {
        name: component_name,
        data () {
            return {
        ...
        methods: {
            ...
            handleQueryInfo (params, isFn, item, success) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['site_sales_curve']);
                }
                ...
                this.getSiteSalesCurveInfo(obj).then(({data}) => {
                    if (!data.status) {
            ...
            handleNodeClick(item, type = 'on') {
                ...
                if (type === 'click') {
                    ...
                    this.$nextTick().then(() => {
                        const itemTop = this.$refs[`${item.code}`][0].$el.getBoundingClientRect().top;
