        ...
        <TemplateDefault :staticNum="120" color="#FFFFFF">
            <div class="ui-content_area clearFix">
                ...
                <div class="ui-content_area--main">
                    <ReturnOrderCurve v-for="(item, index) in chartData" :key="index" :data="item" :ref="item.target"></ReturnOrderCurve>
                </div>
            </div>
        </TemplateDefault>
    </div>
...
<script>
    ...
    import {
        getOptionsList,
        getAmazonReturnOrderInfo
    } from '@/api/sales_module/amazon_return_order_analysis';

    import ReturnOrderCurve from './AECharts/ReturnOrderCurve.vue'

    // 组件名称，用于keep-alive和权限（要跟路由保持一致）
    const component_name = 'amazon_return_order_analysis';
    export default {
        name: component_name,

        components: {ReturnOrderCurve},
        data () {
            return {
        ...
        methods: {
            ...
            handleQueryInfo (params, isFn, item, success) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['amazon_return_order_analysis']);
                }
                ...
                let obj = {
                    ...
                    isKevin: this.queryData.isKevin,
                };

                this.getAmazonReturnOrderInfo(obj).then(({data}) => {
                    if (!data.status) {
            ...
            handleQueryInfoCallBack ({ data }, item, success, params) {
                ...
                this.$store.commit('SAVE_QUERY_PARAMS_INFO', {
                    key: 'amazon_return_order_analysis',
