                ...
                <ul class="ui-main-module-search clearFix">
                    ...
                    <li>
                        <SearchRequirement label="是否清仓">
                            <el-select size="small" v-model="queryData.isClean">
                                <el-option v-for="item in optionList.comList"
                                           :key="item.value"
                                           :value="item.value"
                                           :label="item.label"></el-option>
        ...
        <TemplateDefault :staticNum="120" color="#FFFFFF">
            <div class="ui-content_area clearFix">
                ...
                <div class="ui-content_area--main">
                    <ComListingEchart :ref="item.target" name="platformProfit" v-for="(item, index) in chartData" width="100%" :key="index" :data="item"></ComListingEchart>
                </div>
            </div>
        </TemplateDefault>
    </div>
...
<script>
    ...
    import {
        getTreeDataList,
    } from '@/api/sales_module/platform_account_sale_curve';

    import {
        getPlatformAccountProfitCurveInfo,
    } from '@/api/sales_module/platform_account_profit_curve';
    ...
    const component_name = 'platform_account_profit_curve';
    export default {
        name: component_name,
...
//        components: {PlatformCurve},
        ...
        methods: {
            ...
            handleQueryInfo (params, isFn, item, success) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['platform_account_profit_curve']);
                }
                ...
                let obj = {
                    ...
                    order_type: this.queryData.status,
                    is_clean: this.queryData.isClean,
                    obj_type: this.queryData.obj_type,
                    apart_id: this.queryData.apart_id,
                    group_id: this.queryData.group_id,
                };

                this.getPlatformAccountProfitCurveInfo(obj).then(({data}) => {
                    if (!data.status) {
...
//                    this.queryData[`${item.type}_id`] = item.key;
            ...
            handleQueryInfoCallBack ({ data }, item, success, params) {
                ...
                this.$store.commit('SAVE_QUERY_PARAMS_INFO', {
                    key: 'platform_account_profit_curve',
