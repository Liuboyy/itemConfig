    ...
    <div class="ui-main-module">
        ...
        <div class="ui-tabs_box">
            <el-tabs v-model="queryData.query_type" type="card" @tab-click="handleTabClick">
                <el-tab-pane label="利润统计" name="1"></el-tab-pane>
                <el-tab-pane label="利润率统计" name="2"></el-tab-pane>
            </el-tabs>
        </div>
...
<script>
    ...
    import {
        getPlatformAccountProfitCountInfo,
    } from '@/api/sales_module/platform_account_profit_count'

    // 组件名称，用于keep-alive和权限（要跟路由保持一致）
    const component_name = 'platform_account_sale_count';
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo (params, isFn, item) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['platform_account_sale_count']);
                }
                ...
                this.getPlatformAccountProfitCountInfo(obj).then(({data}) => {
            ...
            handleQueryInfoCallBack (data, item) {
                ...
                this.$store.commit('SAVE_QUERY_PARAMS_INFO', {
                    key: 'platform_account_sale_count',
                ...
                if (item) {
                    if(item.flag === 'group') {
                        ...
                        item.children.splice(0);
                        this.$nextTick().then(() => {
                            item.children = data.value.map(item => {
                                return {
                                    ...item,
                                    children: [{}],
                                }
                    ...
                    this.$nextTick().then(() => {
                        this.tableData = data.value.map((item, index) => {
                            return index === 0 ? {
                                ...item,
                            } : {
                                ...item,
