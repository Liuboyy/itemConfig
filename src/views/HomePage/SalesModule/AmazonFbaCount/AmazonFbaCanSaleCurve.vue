        ...
        <TemplateDefault :staticNum="120" color="#FFFFFF">
            <div class="ui-content_area clearFix">
                ...
                <div class="ui-content_area--main">
                    ...
                    <FbaCanSaleCurve v-for="(item, index) in chartData" :key="index" :data="item" :ref="item.target"></FbaCanSaleCurve>
                </div>
            </div>
        </TemplateDefault>
    </div>
...
<script>
    ...
    import {
        getTreeDataList,
        getAmazonCanSaleCurveInfo
    } from '@/api/sales_module/amazon_fba_sale_curve';

    import FbaCanSaleCurve from '../AECharts/FbaCanSaleCurve.vue'

    // 组件名称，用于keep-alive和权限（要跟路由保持一致）
    const component_name = 'amazon_fba_sale_curve';
    export default {
        name: component_name,

        components: {FbaCanSaleCurve},
        data () {
            return {
        ...
        methods: {
            ...
            handleQueryInfo (params, isFn, item, success) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['amazon_fba_sale_curve']);
                }
                ...
                this.getAmazonCanSaleCurveInfo(obj).then(({data}) => {
                    if (!data.status) {
            ...
            handleResetQueryInfo () {
                ...
                this.queryData.showLine = ['预计可售卖月份数'];
            },
