...
<script>
    ...
    const toHtml = (contents) => {
        ...
        return contents.reduce((rhtml, ctn) => {
            if (rhtml) {
                let ctnValue = ctn.seriesName === '退单金额占比'|| ctn.seriesName === '退单率' ? ctn.value + '%' : ctn.value;
...
//                    option.yAxis[1].max = this.data.data_max.max_refund_money_rate * 10;
