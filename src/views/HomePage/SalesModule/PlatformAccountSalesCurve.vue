        ...
        <TemplateDefault :staticNum="120" color="#FFFFFF">
            <div class="ui-content_area clearFix">
                ...
                <div class="ui-content_area--main">
                    <ComListingEchart :ref="item.target" name="platformSale" v-for="(item, index) in chartData" width="100%" :key="index" :data="item"></ComListingEchart>
                </div>
            </div>
        </TemplateDefault>
    </div>
...
<script>
    ...
    import {
        getTreeDataList,
        getPlatformAccountSalesCurveInfo
    ...
    const component_name = 'platform_account_sale_curve';
    export default {
        name: component_name,
        ...
        methods: {
            ...
            handleQueryInfo (params, isFn, item, success) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['platform_account_sale_curve']);
                }
                ...
                this.getPlatformAccountSalesCurveInfo(obj).then(({data}) => {
                    if (!data.status) {
...
//                    this.queryData[`${item.type}_id`] = item.key;
            ...
            handleQueryInfoCallBack ({ data }, item, success, params) {
                ...
                this.$store.commit('SAVE_QUERY_PARAMS_INFO', {
                    key: 'platform_account_sale_curve',
