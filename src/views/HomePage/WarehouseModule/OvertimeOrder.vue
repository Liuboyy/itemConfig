<template>
  <div class="ui-main-module">
    <transition name="slide-fade">
      <div class="ui-search-area" v-show="showSearch">
        <ul class="ui-main-module-search clearFix">
          <li>
            <SearchRequirement label="日期" template="splitDate" v-model="queryData.date"></SearchRequirement>
          </li>
          <li>
          ...
          <li>
            <SearchRequirement label="销售小组">
              <el-select
                      ...
                      multiple
                      collapse-tags
                      v-model="queryData.group_id_list"
                      placeholder="请选择"
                      size="small"
              >
                <el-option
                        v-for="item in optionList.salesteamList"
                        :key="item.value"
                        :label="item.label"
                        :value="item.value"
                ></el-option>
        ...
        <div class="ui-filter-btn">
          ...
          <el-button size="small" type="primary" icon="icon-chaxun" @click="handleQueryInfo">查询</el-button>
        </div>
      </div>
    </transition>
    <div class="ui-fn-module">
      ...
      <el-popover placement="right-start" width="650" trigger="hover">
        <div style="font-size: 12px">1.超时订单率=超时订单数/所有进入异常类目订单总数(选择时间段内的)</div>
        <div style="font-size: 12px">2.进入异常时间开始,超过4个小时未处理则为超时订单(周一至周五晚7:30到次日9点为休息时间，不记入异常时间范围)</div>
        <div style="font-size: 12px">3.进入异常时间开始,超过6个小时未处理则为超时订单(周六18点到周一早9点为休息时间，不记入异常时间范围)</div>
        <div style="font-size: 12px">4.中午12:00 — 13:30为休息时间，不计入异常时间范围</div>
    ...
    >
      <template slot="timeout_count" slot-scope="scope">
        <span
                @click.stop="handClickIfon(scope.row, 'timeout_count')"
                class="ui-link"
        >{{scope.row.timeout_count}}</span>
      </template>
      <template slot="abnormal_count" slot-scope="scope">
        <span
                @click.stop="handClickIfon(scope.row, 'abnormal_count')"
                class="ui-link"
        >{{scope.row.abnormal_count}}</span>
      </template>
    </ComTableInfo>
    ...
    <ComDialogPreview
            ...
            @export="handleExportDetailsInfo"
    ></ComDialogPreview>
...
<script>
    ...
    import { autoAddTableInx, postExcelFile } from "@/services";
    import { mapActions, mapGetters } from "vuex";
    import {
        getOvertimeOrderInfo,
        getOvertimeList,
        getExceptionDetails
    } from "@/api/warehouse_module/overtime_order";

    import { getCountDate, validTime } from "@/api/common/date_format";

    const component_name = "overtime_order";
    export default {
        name: component_name,
        data() {
            return {
        ...
        methods: {
            ...mapActions(["GetStatiscalReportInfo"]),
            autoAddTableInx,
            getOvertimeOrderInfo,
            ...
            handleQueryInfo(params = {}, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo["overtime_order"]);
                }
                ...
                this.getOvertimeOrderInfo(obj).then(({ data }) => {
            ...
            handleResetQueryInfo() {
                ...
                this.queryData.group_id_list = [];
            },

            // 弹框明细
            handClickIfon(row, type, page, params) {
                this.saveData.row = row;
                this.saveType = type;
                switch (type) {
                    case "timeout_count":
                        this.disleiName = '超时数量弹窗'
                        let obj = {
                            ...this.queryData,
                            ...params,
                            ...page,
                        ...
                        getExceptionDetails(obj).then(({ data }) => {
                            if (data.status === 2) return;
                            if (data.status) {
                                this.dialogVisible = true;
                                this.dialog.key = ["序号", ...Object.values(data.key)];
                                this.dialog.props = ["index", "platform_code", "order_id", "platform_order_id", "reason", "buyer_id", "account_name", "group_name", "paytime", "abnormal_time", "timeout_time", "abnormal_type", "site", "warehouse_name", "bigwarehouse"
                                ];
                                this.dialog.data = data.value;
                                this.dialog.width = this.dialog.props.map(item =>{
                                    if(item === 'index') return '80'
                                    if(item === 'order_id') return '150'
                                    if(item === 'platform_order_id') return '130'
                                    if(item === 'abnormal_type') return '120'
                                    if(item === 'buyer_id') return '160'
                                    if(item === 'account_name') return '150'
                                    if(item === 'group_name') return '100'
                                    if(item === 'paytime') return '140'
                                    if(item === 'abnormal_time') return '140'
                                    if(item === 'timeout_time') return '150'
                                    if(item === 'warehouse_name') return '150'
                                    if(item === 'reason') return '150'
                                })

                                this.dialogPageData.limit = data.limit * 1;
                                this.dialogPageData.offset = data.offset * 1;
                                this.dialogPageData.total = data.total * 1;
                            } else {
                    ...
                    case "abnormal_count":
                        ...
                        getExceptionDetails(objs).then(({ data }) => {
                            if (data.status === 2) return;
                            if (data.status) {
                                this.dialogVisible = true;
                                this.dialog.key = ["序号", ...Object.values(data.key)];
                                this.dialog.props = ["index", "platform_code", "order_id", "platform_order_id", "abnormal_type", "buyer_id", "account_name", "group_name", "paytime", "abnormal_time", "timeout_time", "abnormal_type", "site", "warehouse_name", "bigwarehouse"
                                ];
                                this.dialog.data = data.value;
            ...
            handleExportDetailsInfo() {
                let obj = {
                    ...
                    start_time: this.queryData.date[0] || "",
                    end_time: this.queryData.date[1] || "",
                    platform_code:this.dialog.data[0].platform_code,
                    type: this.saveType
                };
                let str = '?';
                for (let key in obj) {
                ...
                // window.open('http://39.108.55.184:8802/yibai/zhiku/order/timeout/detail1/list/down/' + str, '_self');
                window.open(`${config.BI_PATH2}/yibai/zhiku/order/timeout/detail1/list/down/` + str, '_self');
            },

            //弹框分页
            handleDetailsPageChange({ offset, limit }) {
                this.saveData.page = { offset, limit };
                this.handClickIfon(this.saveData.row, this.saveType, { offset, limit }, {});
            },
            ...
            handleQueryInfoCallBack(data) {
                ...
                let length = !this.queryData.abnormity_superclass_list.length
                    ? 22
                    : this.queryData.abnormity_superclass_list.length;
                let newProps = props.slice(2, 2 + length);
                let keys = newProps.map(item => {
                ...
                //                this.tableProps = ['index', 'warehouse_name', 'platform_name', '1', '2', '3', '4', '5', '66', '9', '11', '12', '15', '18', '19', '20', '21', '22', '120', '16', '14', '111', '23', '13', '110'];
        ...
        created() {
            ...
            this.queryData.date = [getCountDate(-7), getCountDate(-1)];
            this.handleQueryInfo();
            getOvertimeList().then(({ data }) => {
                this.optionList.salesteamList = data.groups;
            });
        }
    };
</script>
