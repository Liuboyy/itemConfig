...
<script>
    ...
    import {
        getSupplierSalesBusinessInfo,
        getOptionList,
        getProductLineList,
        getVariableAndTimeList,
        getAllEditTitleInfo,
        getSaveEditTitleInfo
    } from '../../../api/supplier_module/supplier_sales_business_report'


    // 组件名称，用于keep-alive和权限（要跟路由保持一致）
    const component_name = 'supplier_sales_business_report';
    ...
    export default {
        ...
        methods: {
            ...
            handleResetQueryInfo(){
                ...
                this.queryData.variableType = 'devp_cnt';
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['supplier_sales_business_report']);
                }
                let reg = /^\d+(\.\d+)?$/;
                ...
                this.getSupplierSalesBusinessInfo(obj).then(({ data}) => {
            ...
            handleSortChange(data) {
                ...
                this.$store.commit('SAVE_QUERY_PARAMS_INFO', {
                    key: 'supplier_sales_business_report',
            ...
            handleQueryInfoCallBack(data){
                ...
                this.tableMinWidth = this.tableProps.map(item => {
                    ...
                    if(item === 'sale_cnt_3'|| item === 'sale_cnt_7') return '110';
                    if(item === 'one_month_predict_num'|| item === 'six_month_predict_num') return '120';
                    if(item === 'sale_cnt_15'|| item === 'sale_cnt_30'||item === 'sale_cnt_60'|| item === 'sale_cnt_90') return '120';
                    if(item === 'sale_amt_3'|| item === 'sale_amt_7') return '130';
                    if(item === 'devp_cnt_180'|| item === 'devp_cnt_360') return '150';
        ...
        created(){
            ...
            try{
                ...
                getVariableAndTimeList({ type: 1 }).then(({ data }) => {
