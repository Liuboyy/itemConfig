...
<script>
    ...
    import {
        ...
        getOptionList,
        getProductLineList
    } from '../../../api/supplier_module/supplier_product_report'


    // 组件名称，用于keep-alive和权限（要跟路由保持一致）
    const component_name = 'supplier_product_report';
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['supplier_product_report']);
                }
                let obj = {
                ...
                this.getSupplierProductInfo(obj).then(({ data}) => {
            ...
            handleQueryInfoCallBack(data){
                ...
                this.tableProps = ['index', 'supplier_code', 'supplier_name', 'category_name','all_sku_cnt','on_sale_sku_cnt','stop_sale_sku_cnt','changing_sku_cnt', 'developing_sku_cnt','other_sku_cnt','gn_sku_cnt','hw_sku_cnt','sku_rate', 'status_name', 'category_type_name', 'supplier_resource_name', 'settle_method_name', 'create_time'];
                ...
                this.tableMinWidth = this.tableProps.map(item => {
                    ...
                    if(item === 'status_name'|| item === 'category_type_name'|| item === 'supplier_resource_name'|| item === 'settle_method_name'|| item === 'all_sku_cnt') return '100';
                    if(item === 'sku_rate'|| item === 'other_sku_cnt') return '110';
                    if(item === 'create_time'|| item === 'developing_sku_cnt') return '150';
                    if(item === 'supplier_name') return '200';
                    return '120';
                });
                this.tableSort = this.tableProps.map(item => {
                    if(item === 'all_sku_cnt'|| item === 'on_sale_sku_cnt'|| item === 'stop_sale_sku_cnt'|| item === 'changing_sku_cnt'|| item === 'developing_sku_cnt'|| item === 'other_sku_cnt'|| item === 'gn_sku_cnt'|| item === 'hw_sku_cnt'|| item === 'sku_rate') return 'custom';
                    return false;
                });
