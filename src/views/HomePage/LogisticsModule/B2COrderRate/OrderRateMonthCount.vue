    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['order_rate_month']);
                }
                ...
                this.GetOrderRateMonthCountInfo(obj).then(({ data }) => {
            ...
            handleClearPlatform() {
                ...
                this.optionList.accountNameList = [];
            },
            ...
            handleQueryInfoCallBack(data) {
                ...
                this.$store.commit('SAVE_QUERY_PARAMS_INFO', {
                    key: 'order_rate_month',
                    value: JSON.stringify(this.queryData)
                });

                //渲染数据
                this.tableKey = ['序号', ...data.data.key];
                this.tableProps = ['index','month','platform_name','finish_rate','check_rate','abnormal_rate','push_rate','dispatched_rate','upload_rate','draw_rate','packag_rate','warehouse_rate','freight_rate'];
                ...
                this.tableWidth = this.tableProps.map(item => {
                    if(item === 'index') return '70';
                    if(item === 'month') return '85';
                    if(item === 'dispatched_rate') return '110';
                    if(item === 'abnormal_rate') return '120';
                    if(item === 'upload_rate'|| item === 'draw_rate'|| item === 'packag_rate'|| item === 'warehouse_rate'|| item === 'platform_name') return '150';
                    return '140';
                });
        ...
        created() {
            ...
            try{
                getMonthRange().then((data) => {
                    ...
                    if (JSON.stringify(this.orderRateMonthCountInfo) === '{}') {
                        this.queryData.months = this.saveDefaultTime;
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['order_rate_month']);
                        this.handleQueryInfoCallBack(this.orderRateMonthCountInfo);
                        if(this.comOptions['month_count']) {
                            this.optionList = this.comOptions['month_count'];
                        }
                    }
                });
            }catch (e) {
