    ...
    <div  class="ui-main-module">
        <transition name="slide-fade">
            <div class="ui-search-area" v-show="showSearch">
                <ul class="ui-main-module-search clearFix">
                    ...
                    <li>
                        <SearchRequirement label="账号名称">
                            <el-select collapseTags multiple clearable filterable v-filter v-model="queryData.accountName" placeholder="请选择" size="small">
                                <el-option
                    ...
                    <li>
                        <SearchRequirement label="订单">
                            <el-input size="small" v-model="queryData.orderNo" placeholder="请输入"></el-input>
                        </SearchRequirement>
                    </li>
                    <li>
                        <SearchRequirement label="订单状态">
                            <el-select collapseTags multiple clearable filterable v-filter v-model="queryData.orderStatus" placeholder="请选择" size="small">
                                <el-option
                    ...
                    <li>
                        <SearchRequirement label="物流商">
                            <el-select collapseTags multiple clearable filterable v-filter v-model="queryData.logisticsProvider" placeholder="请选择" size="small"
                                       @change="handleChangeLogisticsProvider" @clear="handleClearLogisticsProvider">
                                <el-option
                    ...
                    <li>
                        <SearchRequirement label="物流方式">
                            <el-select collapseTags multiple clearable filterable v-filter v-model="queryData.logisticsWay" placeholder="请选择" size="small">
                                <el-option
                    ...
                    <li>
                        <SearchRequirement label="发货大仓">
                            <el-select collapseTags multiple clearable filterable v-filter v-model="queryData.shipOkura" placeholder="请选择" size="small"
                                       @change="handleChangeShipOkura" @clear="handleClearShipOkura">
                                <el-option
                    ...
                    <li>
                        <SearchRequirement label="发货仓库">
                            <el-select collapseTags multiple clearable filterable v-filter v-model="queryData.shipWarehouse" placeholder="请选择" size="small">
                                <el-option
                    ...
                    <li>
                        <SearchRequirement label="站点">
                            <el-select collapseTags multiple clearable filterable v-filter v-model="queryData.site" placeholder="请选择" size="small">
                                <el-option
                ...
                <FilterBtn  :resetFn="handleResetQueryInfo" :queryFn="handleQueryInfo" refTable="tableArea"></FilterBtn>
            </div>
        </transition>
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['order_rate_day']);
                }
                if(!validTime(this.queryData.paymentTime[0], this.queryData.paymentTime[1])) {
                    this.$message.warning('结束时间不能小于开始时间，请重新选择');
                    return;
                }
                let obj = {
                    ...params,
                    pay_time_start: this.queryData.paymentTime[0],
                    pay_time_end: this.queryData.paymentTime[1],
                ...
                this.GetOrderRateDayCountInfo(obj).then(({ data }) => {
            ...
            handleResetQueryInfo() {
                this.queryData.paymentTime = [getCountDate(-7), getCountDate(-1)];
                this.queryData.platform = '';
                this.queryData.accountName = [];
                this.queryData.orderNo = '';
                this.queryData.orderStatus = ['20'];
                this.queryData.logisticsProvider = [];
                this.queryData.logisticsWay = [];
                this.queryData.shipOkura = ['1'];
                this.queryData.shipWarehouse = [];
                this.queryData.site = [];
                this.queryData.isStock = '0';
            ...
            handleChangePayTime(val) {
                ...
                if(end - start > time) {
                    this.$message.warning('查询日期不能大于30天！')
                }
            },
            ...
            handleQueryInfoCallBack(data) {
                ...
                this.tableProps = ['index','date','week_day','platform_name','finish_rate','check_rate','abnormal_rate','push_rate','dispatched_rate','upload_rate','draw_rate','packag_rate','warehouse_rate','freight_rate'];
                ...
                this.tableMinWidth = this.tableProps.map(item => {
                    ...
                    if(item === 'week_day') return '70';
                    if(item === 'date') return '90';
                    if(item === 'dispatched_rate') return '110';
                    if(item === 'abnormal_rate'|| item === 'platform_name') return '120';
                    if(item === 'check_rate'|| item === 'push_rate') return '130';
...
//                this.pageData.currentPage = data.page_data.page * 1 ;
        ...
        created() {
            this.thisVm = this;
//            this.saveDefaultTime = [getCountDate(-7), getCountDate(-1)];
            this.handleGetOptionList();
            try{
                if (JSON.stringify(this.orderRateDayCountInfo) === '{}') {
                    this.queryData.paymentTime = [getCountDate(-7), getCountDate(-1)];
                    this.handleQueryInfo();
                } else {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['order_rate_day']);
                    this.handleQueryInfoCallBack(this.orderRateDayCountInfo);
                    if(this.comOptions['day_count']) {
                        this.optionList = this.comOptions['day_count'];
                    }
                }
            }catch (e) {
                console.log(e);
