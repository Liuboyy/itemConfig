    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['order_week_aging']);
                }
                ...
                this.GetOrderExecutionAgingWeekInfo(obj).then(({ data }) => {
            ...
            handlePreviewPlatform(row, params) {
                let obj = {
                    detail_type: 2,
                    paytime_year: row.paytime_year,
                    natural_num: row.natural_num,
            ...
            handleQueryInfoCallBack(data) {
                ...
                this.tableProps = ['index','paytime_year','natural_num','platform_name','check_use_time','abnormal_use_time','handle_use_time','push_use_time','upload_use_time','dispatched_use_time','draw_use_time','packag_use_time','warehouse_use_time','freight_use_time_a','freight_use_time_b','paytime_use_time','onnet_freight_time','onnet_pay_time'];

                this.tableMinWidth = this.tableProps.map(item => {
                    if(item === 'index'|| item === 'paytime_year'|| item === 'natural_num') return '70';
                    if(item === 'dispatched_use_time'|| item === 'status_name'|| item === 'warehouse_code_name') return '90';
                    if(item === 'check_use_time'|| item === 'abnormal_use_time'|| item === 'push_use_time'|| item === 'handle_use_time') return '100';
                    if(item === 'upload_use_time'|| item === 'account_name'||item === 'platform_name') return '110';
        ...
        created() {
            ...
            try{
                getCurrentYear().then((data) => {
                    ...
                    if (JSON.stringify(this.orderExecutionAgingWeekInfo) === '{}') {
                        this.queryData.year = this.saveDefaultTime;
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['order_week_aging']);
                        this.handleQueryInfoCallBack(this.orderExecutionAgingWeekInfo);
                        if(this.comOptions['week']) {
                            this.optionList = this.comOptions['week'];
                        }
                    }
