    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['order_month_aging']);
                }
                ...
                this.GetOrderExecutionAgingMonthInfo(obj).then(({ data }) => {
            ...
            handleQueryInfoCallBack(data) {
                ...
                this.tableProps = ['index','paytime_month','platform_name','check_use_time','abnormal_use_time','handle_use_time','push_use_time','upload_use_time','dispatched_use_time','draw_use_time','packag_use_time','warehouse_use_time','freight_use_time_a','freight_use_time_b','paytime_use_time','onnet_freight_time','onnet_pay_time'];
        ...
        created() {
            ...
            try{
                getMonthRange().then((data) => {
                    ...
                    if (JSON.stringify(this.orderExecutionAgingMonthInfo) === '{}') {
                        this.queryData.months = this.saveDefaultTime;
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['order_month_aging']);
                        this.handleQueryInfoCallBack(this.orderExecutionAgingMonthInfo);
