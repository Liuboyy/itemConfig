                ...
                <ul class="ui-main-module-search clearFix">
                    ...
                    <li>
                        <SearchRequirement label="是否缺货订单">
                            <el-select v-model="queryData.isStock" placeholder="请选择" size="small" clearable>
                                <el-option
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['order_day_aging']);
                }
                ...
                let obj = {
                    ...
                    ship_country: this.queryData.site.join(','),
                };

                obj = Object.assign(params || {}, obj);

                this.GetOrderExecutionAgingDayInfo(obj).then(({ data }) => {
            ...
            handlePreviewPlatform(row, params) {
                let obj = {
                    ...
                    ship_country: row.ship_country.join(','),
            ...
            handlePreviewPlatformCallBack(data) {
                ...
                this.$nextTick().then(() => {
                    ...
                    this.platformWidth = this.platformProps.map(item => {
                        ...
                        if(item === 'is_lack_stock'|| item === 'country_name') return '100';
            ...
            handleQueryInfoCallBack(data) {
                ...
                this.tableData = data.data.value.map(item => {
                    return {
                        ...
                        ship_country: this.queryData.site,
                        is_lack_stock: this.queryData.isStock,
                    }
                });
                this.tableProps = ['index','paytime','week_day','platform_name','check_use_time','abnormal_use_time','handle_use_time','push_use_time','upload_use_time','dispatched_use_time','draw_use_time','packag_use_time','warehouse_use_time','freight_use_time_a','freight_use_time_b','paytime_use_time','onnet_freight_time','onnet_pay_time'];

                this.tableMinWidth = this.tableProps.map(item => {
                    if(item === 'index'|| item === 'week_day') return '70';
                    if(item === 'paytime'|| item === 'week_day'|| item === 'account_name'|| item === 'dispatched_use_time'|| item === 'status_name'|| item === 'warehouse_code_name') return '90';
                    if(item === 'check_use_time'|| item === 'abnormal_use_time'|| item === 'push_use_time'|| item === 'handle_use_time') return '100';
                    if(item === 'upload_use_time'|| item === 'platform_name'|| item === 'account_name') return '110';
                    if(item === 'onnet_freight_time'|| item === 'onnet_pay_time') return '140';
                    if(item === 'warehouse_name') return '150';
                    if(item === 'ship_name') return '250';
                    return '120'
                });
...
//            this.saveDefaultTime = [getCountDate(-7), getCountDate(-1)];
            ...
            try{
                if (JSON.stringify(this.orderExecutionAgingDayInfo) === '{}') {
                    this.queryData.paymentTime = [getCountDate(-7), getCountDate(-1)];
                    this.handleQueryInfo();
                } else {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['order_day_aging']);
                    this.handleQueryInfoCallBack(this.orderExecutionAgingDayInfo);
                    if(this.comOptions['day_aging']) {
                        this.optionList = this.comOptions['day_aging'];
                    }
                }
            }catch (e) {
                console.log(e);
