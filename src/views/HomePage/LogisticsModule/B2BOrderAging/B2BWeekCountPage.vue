    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['b2b_week_aging']);
                }
                ...
                this.GetB2BOrderAgingWeekInfo(obj).then(({ data }) => {
            ...
            handleQueryInfoCallBack(data) {
                ...
                this.tableProps = ['index','paytime_year','natural_num','platform_type', 'order_sum' ,'push_use_time','logistics_audit_use_time','dispatched_use_time','draw_order_use_time','scan_packag_use_time','scan_warehouse_use_time','freight_use_time','total_use_time'];

                this.tableMinWidth = this.tableProps.map(item => {
                    if(item === 'index'|| item === 'paytime_year'|| item === 'platform_type'|| item === 'natural_num') return '70';
        ...
        created() {
            ...
            try{
                getCurrentYear().then((data) => {
                    ...
                    if (JSON.stringify(this.b2bOrderAgingWeekInfo) === '{}') {
                        this.queryData.year = this.saveDefaultTime;
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['b2b_week_aging']);
                        this.handleQueryInfoCallBack(this.b2bOrderAgingWeekInfo);
                        if(this.comOptions['week_aging']) {
                            this.optionList = this.comOptions['week_aging'];
                        }
                    }
                });
            }catch (e) {
