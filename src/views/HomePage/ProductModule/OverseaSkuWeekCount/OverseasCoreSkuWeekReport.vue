...
<script>
    ...
    const component_name = 'oversea_sku_week_count';
    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['core_sku_report']);
                }
                ...
                this.GetCoreOverseaSkuWeekReportInfo(obj).then(({ data }) => {
        ...
        created() {
            ...
            try {
                if (JSON.stringify(this.overseaCoreSkuWeekReportInfo) === '{}') {
                    this.handleQueryInfo();
                } else {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['core_sku_report']);
                    this.defaultSort = this.queryData.sortOrder;
                    this.handleQueryInfoCallBack(this.overseaCoreSkuWeekReportInfo);
                }
            } catch (e) {
                console.log(e);
            }
