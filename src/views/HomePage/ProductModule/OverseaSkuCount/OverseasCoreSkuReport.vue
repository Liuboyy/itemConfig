    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                this.GetCoreOverseaSkuReportInfo(obj).then(({ data }) => {
        ...
        created() {
            ...
            try {
                if (JSON.stringify(this.overseaCoreSkuReportInfo) === '{}') {
                    this.handleQueryInfo();
                } else {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['core_sku_report']);
                    this.defaultSort = this.queryData.sortOrder;
                    this.handleQueryInfoCallBack(this.overseaCoreSkuReportInfo);
                }
            } catch (e) {
                console.log(e);
            }
