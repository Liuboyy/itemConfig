    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['copy_product_line_statistics']);
                }
                ...
                this.GetCopyProductLineInfo(obj).then(({ data}) => {
                    // 导出
                    if (!data.status) {
            ...
            handleQueryInfoCallBack(data){
                ...
                this.$store.commit('SAVE_QUERY_PARAMS_INFO', {
                    key: 'copy_product_line_statistics',
                ...
                this.tableProps = ['p_level_name','avg_task_time'];
                let xAxisArr = [];
                let seriesArr = [];
                this.tableData.forEach(item => {
                    xAxisArr.push(item.p_level_name);
        ...
        created(){
            ...
            try{
                getPreDateRange().then((data) => {
                    if (JSON.stringify(this.copyProductLineCountInfo) === '{}') {
                        this.queryData.date = [data.start, data.end];
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['copy_product_line_statistics']);
                        this.handleQueryInfoCallBack(this.copyProductLineCountInfo);
                    }
                });
            }catch (e) {
                console.log(e);
