            ...
            <div class="ui-search-area" v-if="showSearch">
                <ul class="ui-main-module-search clearFix">
                    ...
                    <li v-if="queryData.view_type === 'd'">
                        <SearchRequirement label="统计日期" template="splitDate" :pickerOptions="pickerOptions" v-model="queryData.date">
                        </SearchRequirement>
                    </li>
                    <li v-if="queryData.view_type === 'm'">
                        <SearchRequirement label="统计月份" template="splitMonth" :pickerOptions="pickerOptions" v-model="queryData.date">
                        </SearchRequirement>
                    </li>
    ...
    export default {
        ...
        methods: {
            ...
            handleChangeType(val) {
                switch (val) {
                    case 'd':
                        ...
                        console.log('d'+this.saveDefaultDate)
                        break;
                    case 'm':
                        this.queryData.date = this.saveDefaultMonth;
                        console.log('m'+this.saveDefaultMonth)
                        break;
                    case 'y':
                        ...
                        console.log('m'+this.queryData.date[0])
                         console.log('m'+typeof(this.queryData.date[0]))
                        break;
                }
            },
            // 重置
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['copy_group_team']);
                }
                ...
                this.GetCopyGroupTeamInfo(obj).then(({ data}) => {
                    // 导出
                    ...
                    if(type=='2'){
                        ...
                        if(data.data.length){
                            ...
                            data.data.forEach(item=>{
                                ...
                                xAxisData[0].data.push(item.group_name)
                            })
        ...
        created(){
            ...
            try{
                getProductLineList().then(({ data }) => {
                    ...
                    console.log(this.optionList.product_line)
                ...
                getYearRange().then(( data ) => {
                    ...
                    getMonthRange().then(( data ) => {
                        console.log('Month')
                        this.saveDefaultMonth = [data.start, data.end];
                        console.log(this.saveDefaultMonth)
                        getPreDateRange().then((data) => {
                             this.saveDefaultDate = [data.start, data.end];
                             this.queryData.date = [data.start, data.end];
                            if (JSON.stringify(this.copyGroupTeamInfo.tables) === '{}') {
                                this.handleQueryInfo();
                            } else {
                                // 获取保存的查询参数
                                this.queryData = JSON.parse(this.queryParamsInfo['copy_group_team']);
                                this.handleQueryInfoCallBack(this.copyGroupTeamInfo.tables);
                                this.setEchar(this.copyGroupTeamInfo.echart1)
                                this.statYear=this.queryData.date[0].toString();
                                this.endYear=this.queryData.date[1].toString();
                            }
                        });
