    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['copy_group_member']);
                }
                ...
                this.GetCopyGroupMemberInfo(obj).then(({ data}) => {
                    // 导出
                    ...
                    if(type=='2'){
                        ...
                        if(data.data.length){
                            ...
                            data.data.forEach(item=>{
                                ...
                                xAxisData[0].data.push(item.user_name)
                            })
                ...
                getYearRange().then(( data ) => {
                    ...
                    getMonthRange().then(( data ) => {
                        ...
                        getPreDateRange().then((data) => {
                                ...
                                this.queryData.date = [data.start, data.end];
                            if (JSON.stringify(this.copyGroupMemberInfo.tables) === '{}') {
                                this.handleQueryInfo();
                            } else {
                                // 获取保存的查询参数
                                this.queryData = JSON.parse(this.queryParamsInfo['copy_group_member']);
                                this.handleChangeGetStaff(this.queryData.group_list);
                                this.handleQueryInfoCallBack(this.copyGroupMemberInfo.tables);
                                this.setEchar(this.copyGroupMemberInfo.echart1)
                                this.statYear=this.queryData.date[0].toString();
                                this.endYear=this.queryData.date[1].toString();
                                if (this.comOptions['copy_member']) this.optionList = this.comOptions['copy_member'];
                            }
                        });
                    })
                })
