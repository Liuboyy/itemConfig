    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['copy_group_productLine']);
                }
                ...
                this.GetCopyGroupProductLineInfo(obj).then(({ data}) => {
                    // 导出
                    ...
                    if(type=='2'){
                        ...
                        if(data.data.length){
                            ...
                            data.data.forEach(item=>{
                                ...
                                xAxisData[0].data.push(item.category_name)
                            })
                    ...
                    getMonthRange().then(( data ) => {
                        ...
                        getPreDateRange().then((data) => {
                            this.saveDefaultDate = [data.start, data.end];
                             this.queryData.date = [data.start, data.end];
                            if (JSON.stringify(this.copyGroupProductLineInfo.tables) === '{}') {
                                this.handleQueryInfo();
                            } else {
                                // 获取保存的查询参数
                                this.queryData = JSON.parse(this.queryParamsInfo['copy_group_productLine']);
                                this.handleChangeGetStaff(this.queryData.group_list);
                                this.handleQueryInfoCallBack(this.copyGroupProductLineInfo.tables);
                                this.setEchar(this.copyGroupProductLineInfo.echart1)
                                this.statYear=this.queryData.date[0].toString();
                                this.endYear=this.queryData.date[1].toString();
                                if (this.comOptions['copy_productLine']) this.optionList = this.comOptions['copy_productLine'];
                            }
                        });
                    })
                })
