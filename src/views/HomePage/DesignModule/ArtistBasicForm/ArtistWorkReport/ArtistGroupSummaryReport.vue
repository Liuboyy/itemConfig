    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['artist_group_summary']);
                }
                ...
                this.GetArtistGroupSummaryReportInfo(obj).then(({ data}) => {
                    // 导出
...
//                    total: this.pageData.currentTotal
        ...
        created(){
            ...
            try{
                getPreDateRange().then((data) => {
                    if (JSON.stringify(this.artistGroupSummaryReportInfo) === '{}') {
                        this.queryData.date = [data.start, data.end];
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['artist_group_summary']);
                        this.handleQueryInfoCallBack(this.artistGroupSummaryReportInfo);
                    }
                });
            }catch (e) {
                console.log(e);
