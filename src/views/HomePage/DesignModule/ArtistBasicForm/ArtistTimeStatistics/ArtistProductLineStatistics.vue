    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['artist_product_line_count']);
                }
                ...
                this.GetArtistProductLineTimeCountInfo(obj).then(({ data}) => {
                    // 导出
                    if (!data.status) {
            ...
            handleQueryInfoCallBack(data){
                ...
                this.$store.commit('SAVE_QUERY_PARAMS_INFO', {
                    key: 'artist_product_line_count',
                    value: JSON.stringify(this.queryData)
                });

                //数据渲染
                this.optionList.productLineList = data.list.products;
                this.tableData = data.list.value;
                this.tableKey = data.list.key;
                this.tableProps = ['category_name_level_1','avg_date'];
        ...
        created(){
            ...
            try{
                getPreDateRange().then((data) => {
                    if (JSON.stringify(this.artistProductLineTimeCountInfo) === '{}') {
                        this.queryData.date = [data.start, data.end];
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['artist_product_line_count']);
                        this.handleQueryInfoCallBack(this.artistProductLineTimeCountInfo);
                    }
                });
            }catch (e) {
                console.log(e);
