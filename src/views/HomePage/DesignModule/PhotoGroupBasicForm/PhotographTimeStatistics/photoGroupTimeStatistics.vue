    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['photo_group_statistics']);
                }
                ...
                this.GetPhotoGroupTimeCountInfo(obj).then(({ data}) => {
                    // 导出
            ...
            cellStyle(row, column, rowIndex, columnIndex) {
                if(row.row.group_name == '小计'|| row.row.user_name == '小计') {
            ...
            handleQueryInfoCallBack(data){
                ...
                if(this.tableKey.indexOf('摄影人员') > -1) {
                    this.tableProps = ['group_name','user_name','avg_time'];
                }else {
                    this.tableProps = ['group_name','avg_time'];
                }
                this.tableWidth = [];
        ...
        created(){
            ...
            try{
                getPreDateRange().then((data) => {
                    if (JSON.stringify(this.photographGroupTimeCountInfo) === '{}') {
                        this.queryData.date = [data.start, data.end];
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['photo_group_statistics']);
                        this.handleQueryInfoCallBack(this.photographGroupTimeCountInfo);
                        if(this.comOptions['photo_time']) this.optionList = this.comOptions['photo_time'];
                    }
                });
            }catch (e) {
                console.log(e);
