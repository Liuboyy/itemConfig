        ...
        <el-table ref="myTable" :data="tableData" :height="tableHeight" :cell-style="cellStyle" highlight-current-row stripe>
            ...
            <el-table-column label="超时数">
                <el-table-column prop="overtime_spu_cnt" label="SPU" width="70">
                        <span slot slot-scope="scope"
                              :class="buttonList['overtime_num'].permit(thisVm) && scope.row.create_user && scope.row.overtime_spu_cnt > 0 ? 'ui-link' : ''"
            ...
            <el-table-column label="返工数">
                <el-table-column prop="reject_spu_cnt" label="SPU" width="70">
                        <span slot slot-scope="scope"
                              :class="buttonList['reject_num'].permit(thisVm) && scope.row.create_user && scope.row.reject_spu_cnt > 0 ? 'ui-link' : ''"
            ...
            <el-table-column prop="reject_spu_rate" label="返工占比(SPU)" width="100"></el-table-column>
            <el-table-column prop="finish_image_cnt" label="图片完成张数" width="100"></el-table-column>
            <template slot="empty">
                <div>
    ...
    export default{
        ...
        methods: {
            ...
            cellStyle(row, column, rowIndex, columnIndex) {
                if(row.row.user_name == '小计'|| row.row.type_name == '小计') {
