    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['design_summary_report']);
                }
                ...
                this.GetDesignGroupSummaryReportInfo(obj).then(({ data}) => {
                    // 导出
        ...
        created(){
            ...
            try{
                getPreDateRange().then((data) => {
                    if (JSON.stringify(this.designGroupSummaryReportInfo) === '{}') {
                        this.queryData.date = [data.start, data.end];
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['design_summary_report']);
                        this.handleQueryInfoCallBack(this.designGroupSummaryReportInfo);
                    }
                });
            }catch (e) {
                console.log(e);
