    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['design_details_report']);
                }
                ...
                this.GetDesignGroupDetailsReportInfo(obj).then(({ data}) => {
                    // 导出
            ...
            handleExportDetailsInfo(params, type) {
                ...
                switch (type) {
                    case 'user':
                        window.open('/design/Design_work_report/getPersonDetailData' + str, '_self');
                        break;
                    case 'overtime':
                        window.open('/design/Design_work_report/getOverdueDetail' + str, '_self');
                        break;
                    case 'reject':
                        window.open('/design/Design_work_report/getReworkDetail' + str, '_self');
                        break;
                }
            },
            //弹框分页
        ...
        created(){
            ...
            try{
                getPreDateRange().then((data) => {
                    if (JSON.stringify(this.designGroupDetailsReportInfo) === '{}') {
                        this.queryData.date = [data.start, data.end];
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['design_details_report']);
                        this.handleQueryInfoCallBack(this.designGroupDetailsReportInfo);
                    }
                });
            }catch (e) {
                console.log(e);
