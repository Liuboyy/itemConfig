    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['reject_summary']);
                }
                ...
                this.GetRejectSummaryInfo(obj).then(({ data}) => {
                    // 导出
        ...
        created(){
            ...
            try{
                getPreDateRange().then((data) => {
                    ...
                    if (JSON.stringify(this.rejectSummaryInfo) === '{}') {
                        this.queryData.date = this.saveDefaultDate;
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['reject_summary']);
                        this.handleQueryInfoCallBack(this.rejectSummaryInfo);
                    }
                });
            }catch (e) {
                console.log(e);
