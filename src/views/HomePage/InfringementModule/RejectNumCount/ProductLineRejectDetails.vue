    ...
    export default {
        ...
        methods: {
            ...
            handleQueryInfo(params, isFn) {
                ...
                if (isFn) {
                    // 获取保存的查询参数
                    this.queryData = JSON.parse(this.queryParamsInfo['reject_line_details']);
                }
                ...
                this.GetRejectDetailsProductLineInfo(obj).then(({ data}) => {
                    // 导出
            ...
            handleQueryInfoCallBack(data){
                ...
                this.$store.commit('SAVE_QUERY_PARAMS_INFO', {
                    key: 'reject_line_details',
                    value: JSON.stringify(this.queryData)
                });

                let keyList = [...data.title];
                keyList.splice(data.title.indexOf('驳回数') , keyList.length);
        ...
        created(){
            ...
            try{
                ...
                getPreDateRange().then((data) => {
                    ...
                    if (JSON.stringify(this.productLineRejectDetailsInfo) === '{}') {
                        this.queryData.date = this.saveDefaultDate;
                        this.handleQueryInfo();
                    } else {
                        // 获取保存的查询参数
                        this.queryData = JSON.parse(this.queryParamsInfo['reject_line_details']);
                        this.handleQueryInfoCallBack(this.productLineRejectDetailsInfo);
                    }
                });
            }catch (e) {
                console.log(e);
